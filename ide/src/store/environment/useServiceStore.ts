...
import type { ServiceDefinition, AuthDefinition, AuthType } from "@/models/schema";
...
/** Service config schemas — defines available service types and their form fields. */
...
interface ServiceState {
  ...
  getServicesByUses: (uses: string) => ServiceDefinition[];
  hasServiceUses: (uses: string) => boolean;
...
export const useServiceStore = create<ServiceState>((set, get) => ({
  ...
  getServicesByUses: (uses) => get().services.filter((s) => s.uses === uses),

  hasServiceUses: (uses) => get().services.some((s) => s.uses === uses),

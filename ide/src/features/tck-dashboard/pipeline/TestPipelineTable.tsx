...
import { useProjectStore, type ActiveFile, type TestSummary } from "@/store/project/useProjectStore";
  ...
  return (
    ...
    <SectionCard
      ...
      extra={
        <button
          onClick={handleAdd}
          className="pipeline-table__add-btn"
        >
    ...
    >
      {summaries.length === 0 ? (
        <div className="pipeline-table__empty">
          No tests yet. Click "Add Test" to create one.
        </div>
      ) : (
        <div className="pipeline-table__list">
          {summaries.map((summary, index) => (
            <PipelineRow
              ...
              onRemove={() => setPendingDeleteName(summary.name)}
            />
          ))}
        </div>
      )}
    ...
    {pendingDeleteName && (
      <ConfirmDialog
        ...
        confirmLabel="Delete"
        onConfirm={() => { removeTest(pendingDeleteName); setPendingDeleteName(null); }}
...
function PipelineRow({ summary, index, total, isDragging, isDragOver,
  ...
  return (
    ...
    >
      {/* Connector column */}
      <div className="pipeline-row__connector-col">
        <OrderBadge order={summary.order} />
        {!isLast && <div className="pipeline-row__connector-line" />}
      </div>

      {/* Content */}
      <div
        className="pipeline-row__content"
        style={{ borderBottom: isLast ? "none" : `1px solid ${theme.colors.border}` }}
      >
        <DragIndicatorIcon sx={{ fontSize: 14, color: theme.colors.textMuted, opacity: 0.4, cursor: "grab" }} />

        {/* Name + description */}
        <div className="pipeline-row__name-col">
          <button
            onClick={onNavigate}
            className="pipeline-row__name-btn"
          >
            {summary.name}
          </button>
          {summary.description && (
            <div className="pipeline-row__description">
              {summary.description}
            </div>
          )}
          <div className="pipeline-row__description">
        ...
        {/* Step count */}
        <span className="pipeline-row__stat">

...
export interface InputTemplateMeta {
  ...
  label: string;
  description: string;
  /** System this input belongs to, so it surfaces in that system's Add menu. */
  target: SystemId;
...
export const INPUT_TEMPLATES: readonly InputTemplateMeta[] = [
  ...
  },
  {
    id: "dtr_endpoint",
    label: "DTR Endpoint",
    description: "The address of the SUT's Digital Twin Registry to query.",
    target: "dtr",
    ...
    fields: [
      {
        key: "dtr_url",
        label: "DTR base URL",
        placeholder: "https://sut-dtr.example.org/api/v3",
        fieldClass: "url",
      },
    ],
  },

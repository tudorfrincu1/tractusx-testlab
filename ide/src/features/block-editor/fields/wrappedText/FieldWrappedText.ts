...
export class FieldWrappedText extends Blockly.FieldTextInput {
  ...
  /** Single click toggles expansion; click when expanded opens editor. */
  protected override showEditor_(): void {
    if (this.isTruncatable && !this.isExpanded) {
  ...
  private rerenderBlock(): void {
    this.isDirty_ = true;
    const block = this.getSourceBlock();
    if (block && "render" in block) (block as Blockly.BlockSvg).render();
  }
  ...
  override render_(): void {
    ...
    this.isTruncatable = lines.length > MAX_DISPLAY_LINES;
    const isTruncated = this.isTruncatable && !this.isExpanded;

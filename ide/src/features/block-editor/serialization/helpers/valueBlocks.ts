...
import { makeBlock, setDropdownValue } from "./blockUtils";
import { isTemplateString } from "../../fields/templateString/templateStringParser";
...
export function readValueBlockAsString(block: Block | null): string | undefined {
  ...
  if (block.type === "value_template_string") {
    return block.getFieldValue("TEMPLATE") || undefined;
  }
...
export function createValueBlockFromString(ws: Workspace, strVal: string): Block {
  ...
  // Template strings: contain ${{ ... }} interpolation mixed with literal text
  if (isTemplateString(strVal)) {
    const tb = makeBlock(ws, "value_template_string");
    tb.setFieldValue(strVal, "TEMPLATE");
    return tb;
  }

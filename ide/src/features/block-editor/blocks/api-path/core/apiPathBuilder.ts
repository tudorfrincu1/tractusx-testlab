...
export function parseApiPath(path: string): ApiPathSegment[] {
  ...
  return path.replace(/^\//, "").split("/").filter(Boolean).map((part) => {
    const envMatch = ENV_VAR_RE.exec(part);
    if (envMatch) return { type: "variable" as const, value: envMatch[1] };
    if (part.startsWith("@")) return { type: "variable" as const, value: part.slice(1) };
...
export function segmentsToApiPath(segments: ApiPathSegment[]): string {
  ...
  return "/" + segments.map((s) => s.type === "variable" ? `\${{ env.${s.value} }}` : s.value).join("/");
}

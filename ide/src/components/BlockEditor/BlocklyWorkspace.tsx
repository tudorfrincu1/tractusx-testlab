...
import { useToolboxRefresh } from "./useToolboxRefresh";
...
export function BlocklyWorkspace({ onTrashChange }: BlocklyWorkspaceProps) {
  ...
  const { containerRef, ready, ...refs } = useWorkspaceInit(onTrashChangeRef);
  useModelSync(refs, ready);
  useToolboxRefresh(refs, ready);
  useCanvasState(refs, ready);

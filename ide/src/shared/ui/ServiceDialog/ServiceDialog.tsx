...
export function ServiceDialog({ onClose }: ServiceDialogProps) {
  ...
  const handleStartCreate = (type: string) => {
    ...
    setEditTarget({ data: { name, uses: type, with: config }, isCreating: true });
  };

  const handleEdit = (service: ServiceDefinition) => {
    setEditTarget({ data: { ...service, with: { ...service.with } }, isCreating: false });
  };
          ...
          <div style={{
            ...
            {services.map((svc) => (
              <ListItem
                key={svc.name}
                name={svc.name}
                subtitle={SERVICE_SCHEMAS.find((s) => s.type === svc.uses)?.label ?? svc.uses}

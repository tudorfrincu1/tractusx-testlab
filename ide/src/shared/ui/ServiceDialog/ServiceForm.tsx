...
export function ServiceForm({ service, isCreating, onChange, onSave, onCancel }: ServiceFormProps) {
  const schema = SERVICE_SCHEMAS.find((s) => s.type === service.uses);
  if (!schema) return null;

  const serviceWith = service.with ?? {};

  const updateField = (fieldName: string, value: string) => {
    onChange({ ...service, with: { ...serviceWith, [fieldName]: value } });
  };

  return (
    <div className="service-form">
      <div className="service-form__title" style={{ color: theme.colors.textBright }}>
      ...
      {schema.fields.map((field) => (
        <SchemaFieldInput
          key={field.name}
          field={field}
          value={(serviceWith[field.name] as string) ?? ""}
...
function FieldRow({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div className="field-row">
      <label className="field-row__label" style={{ color: theme.colors.textMuted }}>
        {label}
        {required && <span className="field-row__required" style={{ color: theme.colors.primary }}>*</span>}
      </label>
      {children}
    </div>
  );
